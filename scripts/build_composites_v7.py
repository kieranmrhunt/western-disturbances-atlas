...
def build(track_id, output):
    ...
    if path.exists():
        ...
        if existing.get('schema')=='wd-storm-composite-v1' and existing.get('track_id')==track_id and existing.get('build_version')==3:
            return path
    records=json.loads((output.parent/'inputs/index.json').read_text())
    year=next(r[1] for r in records if r[0]==track_id)
    with gzip.open(output.parent/'inputs'/f'{year}.json.gz','rt') as stream:
        row=json.load(stream)[track_id]
    track=pd.DataFrame(row,columns=['time','lat','lon'])
    ...
    indices=np.unique([int(np.argmin(np.abs(track.time.dt.as_unit('ns').astype('int64').to_numpy()-t))) for t in targets])
    for i in indices:
...
def index(output):
    ...
    for track_id,year in zip(ids,catalogue['cat']['year']):
        ...
        if asset.get('schema')!='wd-storm-composite-v1' or asset.get('track_id')!=track_id or asset.get('catalogue')!='WD v7':raise ValueError(path)
    ...
    payload={'schema':'wd-composite-manifest-v1','catalogue':'WD v7','expected_tracks':len(ids),'available_tracks':available,'years':years,'generated_utc':provider.utc_now()}
...
def prepare(output):
    ...
    points=pd.read_parquet(source,columns=['track_id','valid_time_utc','lat','lon']).sort_values(['track_id','valid_time_utc'])
    years=dict(zip(cat['id'],cat['year']))
    shards={}
    for track_id,track in points.groupby('track_id',sort=False):
        rows=[[str(t),float(a),float(b)] for t,a,b in track[['valid_time_utc','lat','lon']].itertuples(index=False,name=None)]
        shards.setdefault(years[track_id],{})[track_id]=rows
    target=output.parent/'inputs'
    target.mkdir(parents=True,exist_ok=True)
    (target/'index.json').write_text(json.dumps(list(zip(cat['id'],cat['year'])),separators=(',',':')))
    for year,tracks in shards.items():
        provider.atomic_gzip_json(tracks,target/f'{year}.json.gz')
    print(f"{len(cat['id'])} prepared composite inputs",flush=True)
...
if __name__=='__main__':
    ...
    p.add_argument('--output',type=Path,default=ROOT/'.v7-products/composites/public')
    args=p.parse_args()
    if args.prepare:prepare(args.output)
    elif args.finalize:index(args.output)
    else:
        records=json.loads((args.output.parent/'inputs/index.json').read_text())
        build(args.track_id if args.track_id is not None else records[args.index+args.offset][0],args.output)
